...
- 列表和相册里只加载封面帧/静帧,视频显示播放角标与时长,实况图显示 LIVE 角标。**一律不会自动播放** —— 视频点一下封面就在原来的格子里播起来(内嵌 ArtPlayer:全屏、画中画、倍速与画面比例设置、键盘快捷键;多列布局里会占满整行),点右上角 × 收回封面;实况图走灯箱,点 LIVE 角标才播放动态部分(带原声)。
