...
本次在浏览器启动现有页面后确认：本地没有动态，首页只剩导航与空状态文字；原有暖色光斑、圆角卡片和渐变按钮的视觉分量接近，缺少明确的站点主体。根据用户偏好，设计方向调整为「玫瑰粉的私人日记」。以下为本次实现，后文保留历史评估。

- **整体视觉**：浅粉白背景、玫瑰粉按钮与选中态、深莓色标题和正文；封面保留青绿远山作为平衡。衬线标题与系统正文字体搭配，保留中文阅读感；取消背景光斑、渐变按钮和动态卡片上浮。
- **首页**：使用横向湖畔风景封面，直接呈现站点名称；封面下为「日常手记」、真实记录数量和原有发布/阅读流。阅读列仍为 640px，月份导航保持外围布局。
- **发布与阅读**：发布框有独立标题、格式工具栏、正文和底部操作行；动态统一使用 8px 圆角与轻边框。更多操作使用 Lucide 三点图标。
- **相册**：加入页标题与真实照片数，桌面采用四列较大的照片，移动端保持三列。照片用原生按钮打开灯箱，可通过键盘操作，并提供作者与时间标签。
- **登录与设置**：登录页复用封面，下方为无卡片包裹的表单和可见字段标签；设置页改为分隔线组织的连续表单，减少卡片堆叠。
- **空状态**：用简短标题、图标和明确的登录/记录入口承接无数据场景，不伪造动态或统计数。

### 封面素材

使用内置 imagegen 生成，压缩后的站内资源为 `public/images/journal-cover.jpg`；无需运行时访问图片服务。最终提示词：保留湖畔全景、右侧树木、草地、青绿远山和平静湖水；将树木改为浅粉色樱花，将光线改为柔和粉色晨光；3:1 横向构图，左侧 60% 留出浅色天空和水面供深莓色标题使用；自然摄影质感，不含人物、建筑、文字、标志或光斑。该图片为默认设计素材，不代表用户真实拍摄的照片。

### 回归验证方法

`node tests/design-regression.mjs` 使用 Node 内置断言和独立的 `data/design-regression-*` 数据目录，执行后清理自身数据。覆盖登录、图文上传与发布、读取、相册、归档、私密访问边界、静态资源和主题配置；不引入测试框架或新依赖。

浏览器检查覆盖首页、登录、发布框、图文动态、月份跳转、相册灯箱、账号与设置，以及桌面和手机宽度下的内容换行。后端接口额外使用 `curl` 执行登录、上传、发布、读取和按月查询冒烟。
